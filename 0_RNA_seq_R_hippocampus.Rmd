...
#PCA Plot
```{r}
tiff(file = 'Figures/PCA/male_Hippocampus_PCA.tiff', units="in", width=8, height=5, res=1000)
PCA <-ggplot(data, aes(PC1, 
                   PC2, 
                   color=exposure,
                   shape= exposure)) + 
  stat_ellipse(aes(color= exposure))+
  ggtitle("Male Hippocampus PCA")+
  geom_point(size=3) +
  scale_color_manual(values = c("#666666", "#FF66CC","#66CCCC")) +
  scale_shape_manual(values = c(19, 15,17)) +
  xlab(paste0("PC1: ",
...
```{r}
tiff(file = "Figures/Male_hippocampus_heatmap.tiff", units="in", width=8, height=5, res=1000)
heatmap<-pheatmap(mat, 
         annotation_col=df,
         labels_row = exp_resOrdered$symbol,
         fontsize = 6,
...
#***volcano plots***
```{r}
tiff(file = 'Figures/volcano/Male_Exposed_volcano_plot_Hipp.tiff', units="in", width=8, height=5, res=1000)
M_vol1<- EnhancedVolcano(res_exp,
    lab = res_exp$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
    title = "Male Exposed Hippocampus",
    subtitle = "",
    ylim = c(0, -log10(10e-8)),
    xlim = c(log2(10e-3), -log2(10e-3)),
    FCcutoff = 1.0,
...
```{r}
exp_resOrdered <- as.data.frame(exp_resOrdered)
write_tsv(exp_resOrdered, file="Results_spreadsheets/exp_male_hip_results.tsv")
write_csv(exp_resOrdered, file="Results_spreadsheets/exp_male_hip_results.csv")
write_csv(as.data.frame(assay(M_se)), file= "Results_spreadsheets/male_hip_counts.csv")
```


```{r}
tiff(file = 'Figures/volcano/male_pair_fed_volcano_plot_hipp.tiff', units="in", width=8, height=5, res=1000)
M_vol1<- EnhancedVolcano(res_pf,
    lab = res_pf$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
    title = "Male Hippocampus",
    subtitle = "Pair Fed ",
    ylim = c(0, -log10(10e-8)),
    xlim = c(log2(10e-3), -log2(10e-3)),
    FCcutoff = 1.0,
...
```{r}
pf_resOrdered <- as.data.frame(pf_resOrdered)
write_tsv(pf_resOrdered, file="Results_spreadsheets/male_pf_Hipp_results.tsv")
write_csv(pf_resOrdered, file="Results_spreadsheets/male_pf_Hipp_results.csv")
```
...
data<-data.frame(groups)
write.csv(data ,file="Figures/Enrichment/Grouped_male_Hipp_Gene_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/Male_Hipp_grouped_gene_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot34<-dotplot(groups,showCategory=3, split=".sign") +
  geom_point(shape = 1,colour = "black") +
  scale_color_viridis(option = "inferno", direction = -1) +
  facet_grid(.~.sign)+
...
data<-data.frame(groups)
write.csv(data ,file="Figures/Enrichment/Male_Grouped_Hipp_KEGG_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/Male_Hipp_grouped_kegg_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot28<-dotplot(groups,showCategory=3, split=".sign") +
  geom_point(shape = 1,colour = "black") +
  scale_color_viridis(option = "inferno", direction = -1) +
  facet_grid(.~.sign)+
...
data<-data.frame(groups)
write.csv(data ,file="Figures/Enrichment/Grouped_Male_Hipp_Disease_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/Male_hipp_grouped_disease_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot32<-dotplot(groups,showCategory=3, split=".sign") +
  geom_point(shape = 1,colour = "black") +
  scale_color_viridis(option = "inferno", direction = -1) +
  facet_grid(.~.sign)+
...
tiff(file = 'Figures/Male_hipp_dot_plots_big.tiff', units="in",width=10, height=15, res=500)
big + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 16, vjust = -10))
dev.off()
...
#***volcano plots***
```{r}
tiff(file = 'Figures/volcano/Female_Exposed_vs_PF_volcano_plot_Hipp.tiff', units="in", width=8, height=5, res=1000)
M_vol1<- EnhancedVolcano(res_F,
    lab = res_F$symbol,
    x = 'log2FoldChange',
    y = 'pvalue',
    title = "Female Exposed vs Pair Fed Hippocampus",
    subtitle = "",
    ylim = c(0, -log10(10e-8)),
    xlim = c(log2(10e-3), -log2(10e-3)),
    FCcutoff = 1.0,
...
write_tsv(F_resOrdered, file="Results_spreadsheets/exp_vs_pf_female_hip_results.tsv")
write_csv(F_resOrdered, file="Results_spreadsheets/exp_vs_pf_female_hip_results.csv")
```


```{r}
tiff(file = 'Figures/volcano/Male_exp_vs_pf_volcano_plot_hipp.tiff', units="in", width=8, height=5, res=1000)
...
write_tsv(M_resOrdered, file="Results_spreadsheets/Male_exp_vs_pf_Hipp_results.tsv")
write_csv(M_resOrdered, file="Results_spreadsheets/Male_exp_vs_pf_Hipp_results.csv")
```
...
data<-data.frame(groups)
write.csv(data ,file="Figures/Enrichment/Grouped_exp_vs_PF_Hipp_Gene_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/Exp_vs_PF_Hipp_grouped_gene_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot34<-dotplot(groups,showCategory=3, split=".sign") +
  geom_point(shape = 1,colour = "black") +
  scale_color_viridis(option = "inferno", direction = -1) +
  facet_grid(.~.sign)+
...
data<-data.frame(groups)
write.csv(data ,file="Figures/Enrichment/Exp_vs_PF_Grouped_Hipp_KEGG_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/Exp_vs_PF_Hipp_grouped_kegg_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot28<-dotplot(groups,showCategory=3, split=".sign") +
  geom_point(shape = 1,colour = "black") +
  scale_color_viridis(option = "inferno", direction = -1) +
  facet_grid(.~.sign)+
...
data<-data.frame(groups)
write.csv(data ,file="Figures/Enrichment/Grouped_exp_vs_pf_Hipp_Disease_enrichment.csv")
```


```{r}
tiff(file = 'Figures/Enrichment/Exp_vs_PF_hipp_grouped_disease_enrichment.tiff', units="in", width=8, height=5, res=1000)
dot32<-dotplot(groups,showCategory=3, split=".sign") +
  geom_point(shape = 1,colour = "black") +
  scale_color_viridis(option = "inferno", direction = -1) +
  facet_grid(.~.sign)+
...
tiff(file = 'Figures/Exp_vs_PF_dot_plots_big.tiff', units="in",width=10, height=15, res=500)
big + plot_annotation(tag_levels = 'A') & 
  theme(plot.tag = element_text(size = 16, vjust = -10))
dev.off()
